...
public class MaxHeap {
    ...
    private int maxChildIndex(int n) {
        if (left(n) > a.size() - 1) 
        	return -1;
        if (right(n) > a.size() - 1) 
        	return left(n);
    ...
    private void bubbleDown(int n) {
        int maxChildIndex = maxChildIndex(n);
        while (maxChildIndex != -1 && a.get(maxChildIndex) > a.get(n)) {
            swap(maxChildIndex, n);
            n = maxChildIndex;
            maxChildIndex = maxChildIndex(n);
        }
    }
